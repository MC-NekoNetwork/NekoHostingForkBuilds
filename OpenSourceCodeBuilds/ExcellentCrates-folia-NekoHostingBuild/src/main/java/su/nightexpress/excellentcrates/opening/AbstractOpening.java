    ...
    protected final void onStop() {
        ...
        if (this.isCompleted()) {
            ...
            if (this.isSaveData() && !this.isEmergency()) {
                this.plugin.runTaskAsync(new Consumer<BukkitTask>() {
                    @Override
                    public void accept(BukkitTask bukkitTask) {
                        CrateUser user = AbstractOpening.this.plugin.getUserManager().getUserData(player);
                        AbstractOpening.this.crate.saveRewardWinDatas();
                        AbstractOpening.this.plugin.getUserManager().save(user);
                    }
                });
            }
        }
